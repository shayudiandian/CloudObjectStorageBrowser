...
{
    QtConcurrent::run([=]() {          // run 方法就是调用的线程池
        try {
...
void GateWay::dispach(int api, const QJsonValue &params)
{
    ...
    case API::LOGIN::NORMAL:
        apiLogin(params); break;

    // 桶操作接口
    case API::BUCKETS::LIST:
        apiGetBuckets(params); break;
    case API::BUCKETS::PUT:
        apiPutBucket(params); break;
    case API::BUCKETS::DEL:
        apiDeleteBucket(params); break;

    // 对象操作接口
    case API::OBJECTS::LIST:
        apiGetObjects(params); break;
    case API::OBJECTS::GET:
        apiGetObject(params); break;
    case API::OBJECTS::PUT:
        apiPutObject(params); break;
